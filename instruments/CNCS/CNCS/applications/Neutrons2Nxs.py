...
appname = 'cncs-neutrons2nxs'
cmd_help = """
convert scattereed neutrons to nexus file.

Examples:

 $ cncs-neutrons2nxs --neutrons=scattered-neutrons-example --nodes=2
"""
...
def sendneutronstodetsys(
    ...
    d = locals()
    cmd = 'mcvine instruments cncs neutrons2events %(neutronfile)s --nodes=%(nodes)s --workdir=%(workdir)s' % d
...
def event2nxs(eventdat, nxs, type, workdir):
    d = dict(globals())
    d.update(locals())
    cmd = 'mcvine instruments cncs events2nxs %(eventdat)s %(nxs)s --type=%(type)s --tofbinsize=%(tofbinsize)s' % d
    execute(cmd, workdir)
    return
