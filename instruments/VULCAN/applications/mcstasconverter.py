...
class McStasConverter:
    ...
    def toString(self, ind=16, br="\n", allparams=True):
        ...
        for comp in self.components():
            ...
            str += "position:%s%s%s" % (self._resIndent("position:", ind), self._formatVector(comp["position"]), br)
            str += "rotation:%s%s%s" % (self._resIndent("rotation:", ind), self._formatVector(comp["rotation"]), br)
    ...
    # XXX: Works incorrectly (replaced by _mcvineVector()), REMOVE?
